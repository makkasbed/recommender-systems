...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items){
        ...
        for (Long item: items) {
            ...
            double userNormalized = 0.0;
            double itemNormalized = 0.0;

            for (Map.Entry<String, Double> e : iv.entrySet())
                itemNormalized += (e.getValue() * e.getValue());

            for (Map.Entry<String, Double> e : userVector.entrySet())
                userNormalized += (e.getValue() * e.getValue());

            // If the denominator of the cosine similarity is 0, skip the item
            if (itemNormalized != 0.0 && userNormalized != 0.0) {
                double userItem = 0.0;

                itemNormalized = Math.sqrt(itemNormalized);
                userNormalized = Math.sqrt(userNormalized);
                ...
                double cosine = userItem / itemNormalized / userNormalized;
